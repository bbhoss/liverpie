  ...
  def receive_data data
    ...
    if @receive_buffer[-2..-1] == "\n\n"
      ...
      unless @initialized
        ...
        @cookie = @client.reset if @initialized
      else   

        if @params['content_type'] == 'text/event-plain'
          @latest_variables = @latest_variables.merge(@params)
        end
        ...
        if @params['dtmf_digit']
          resp, @cookie = @client.send_dtmf(@params['dtmf_digit'], @cookie)
          yaml = YAML.load(resp)
